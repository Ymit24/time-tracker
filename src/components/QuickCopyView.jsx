  ...
  if (entries.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-12 px-4">
        <div
          className="w-12 h-12 rounded-xl bg-[var(--color-surface-1)] border border-[var(--color-surface-3)]
                        flex items-center justify-center mb-3"
        >
          <svg
            className="w-5 h-5 text-[var(--color-ink-2)]"
            fill="none"
            viewBox="0 0 24 24"
            stroke="currentColor"
            strokeWidth={1.5}
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="rounded-xl border border-[var(--color-surface-3)] bg-[var(--color-surface-2)] overflow-hidden font-mono text-xs">
          ...
          return (
            ...
            >
              <div className="flex items-center gap-2">
                ...
                <span className="text-[var(--color-ink-0)] font-semibold">
                  {line.label}:
                </span>
              </div>
              <div className="flex items-center gap-1.5">
                <span className="text-[var(--color-ink-0)] font-bold">
                  {line.rounded.toFixed(2)}
                </span>
                <span className="text-[var(--color-ink-2)]">
                  ({line.exact.toFixed(2)})
                </span>
              </div>
            </div>
          );
        ...
        {/* Total row */}
        <div
          className="flex items-center justify-between px-3 py-2 border-t-2 border-[var(--color-surface-3)]
        ...
        >
          ...
          <div className="flex items-center gap-1.5">
            <span className="text-[var(--color-amber-accent)] font-bold">
              {(() => {
                const ms = calculateSmartTotal(entries);
                return roundTo15Min(ms).toFixed(2);
              })()}
            </span>
            <span className="text-[var(--color-ink-2)]">
              (
